...
O usuário deve conseguir verificar o "SALDO" da sua conta, passando o seu nome e o seu CPF. - ok
 ...
 V   V   V   V
*/
app.get('/users/:name/:cpf', (req, res) => {
    try {
...
app.put('/users', (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < data_1.users.length; i++) {
            if (data_1.users[i].name === name && data_1.users[i].cpf === cpf) {
                result = true;
                data_1.users[i].balance = data_1.users[i].balance + newBalance;
                userData.push(data_1.users[i].balance);
            }
        }
        if (result === false) {
            throw new Error("Usuário não encontrado");
...
*/
app.put('/users/transfer/:name/:cpf', (req, res) => {
    let errorCode = 400;
    try {
        const name = req.params.name;
        const cpf = req.params.cpf;
        ...
        for (let i = 0; i < data_1.users.length; i++) {
            if (data_1.users[i].name === name && data_1.users[i].cpf === cpf) {
                if (data_1.users[i].balance - value < 0) {
                    throw new Error("Saldo Insuficiente");
                }
                else {
                    data_1.users[i].balance = data_1.users[i].balance - value;
                }
            }
            if (data_1.users[i].name === recipientName && data_1.users[i].cpf === recipientCPF) {
                data_1.users[i].balance = data_1.users[i].balance + value;
            }
        }
        res.send(data_1.users);
